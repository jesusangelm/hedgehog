defmodule Naive.Trader do
  ...
  def init(%{symbol: symbol, profit_interval: profit_interval}) do
    ...
    Logger.info("Inicializando nuevo comerciante para #{symbol}")
  ...
  def handle_info(%TradeEvent{price: price}, %State{symbol: symbol, buy_order: nil} = state) do
    quantity = "100" # valor por defecto hasta el capitulo 7

    Logger.info("Colocando orden de COMPRA para #{symbol} @ #{price}, cantidad: #{quantity}")

    {:ok, %Binance.OrderResponse{} = order} = @binance_client.order_limit_buy(symbol, quantity, price, "GTC")
  ...
  def handle_info(
    ...
    %State{symbol: symbol,
           ...
           profit_interval: profit_interval,
           tick_size: tick_size} = state) do

    sell_price = calculate_sell_price(buy_price, profit_interval, tick_size)

    Logger.info("Orden de compra llenada, colocando orden de VENTA para " <>
      "#{symbol} @ #{sell_price}, candidad: #{quantity}")
  ...
  def handle_info(
    ...
    Logger.info("Comercio finalizado, Comerciante terminara ahora")
    {:stop, :normal, state}
  end
